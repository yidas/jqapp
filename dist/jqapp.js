...
;(function (window, $) {
  ...
  var Instance = function (options, el) {
    ...
    // Hook: beforeCreate
    if (typeof this.hook.beforeCreate === 'function') {
      this.hook.beforeCreate.bind(this)();
    }
    ...
    // Hook: created
    if (typeof this.hook.created === 'function') {
      this.hook.created.bind(this)();
    }
  ...
  Instance.prototype.mount = function (elementOrSelector) {
    ...
    // Hook: beforeMount
    if (typeof this.hook.beforeMount === 'function') {
      this.hook.beforeMount.bind(this)();
    }
    ...
    if (elementOrSelector && this.template) {
      ...
      el = el.parentNode.replaceChild(this.template, this.el);
    }
    ...
    // Hook: mounted
    if (typeof this.hook.mounted === 'function') {
      this.hook.mounted.bind(this)();
    }
  ...
  Instance.prototype.remove = function () {
    ...
    // Hook: beforeDestroy
    if (typeof this.hook.beforeDestroy === 'function') {
      this.hook.beforeDestroy.bind(this)();
    }
    ...
    if (this.el.parentNode) {
      ...
      // Hook: destroyed
      if (typeof this.hook.destroyed === 'function') {
        this.hook.destroyed.bind(this)();
      }
    }
  ...
  Instance.prototype.createComponent = function (componentKey, options) {
    ...
    if (options) {
      ...
      if ($) {
        ...
        // Extend options
        options = $.extend(true, {}, this.components[componentKey], options);
         ...
         */
        var tmp = {};
        for (var key in this.components[componentKey])
          if (this.components[componentKey].hasOwnProperty(key))
            tmp[key] = this.components[componentKey][key];
